...
const OVERRIDES = {
  'Bibliotekstorget 4, Solna, Sweden': 'Bibliotekstorget 4, 171 45 Solna, Sweden',
  'Stjärntorget 2 (Plan 1 bredvid SATS), Solna, Sweden': 'Stjärntorget 2, 169 79 Solna, Sweden' // Mall of Scandinavia
}

// 2) Nominatim (OpenStreetMap) geocoder — free, no key.
// Be polite: add a User-Agent and delay between calls.
const GEOCODE = async (q) => {
  const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=1&countrycodes=se&addressdetails=1&accept-language=en`
  const r = await fetch(url, { headers: { 'User-Agent': 'NaimaWebsite/1.0 (contact@naima.example)' } })
  if (!r.ok) return null
  const j = await r.json()
  const f = j?.[0]
  return f ? { lat: Number(f.lat), lng: Number(f.lon), label: f.display_name } : null
}
...
for (const item of input) {
  ...
  if (!res) { console.warn('No match:', query); continue }
  out.push({
    ...item,
...
console.log('Geocoded', out.length, 'locations ->', outputPath)
